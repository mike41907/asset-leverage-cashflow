  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <div className="modal-card loan-modal-card" role="dialog" aria-modal="true" aria-labelledby="liability-modal-title">
        <div className="modal-header"><div><div className="section-kicker">一般負債資料</div><h2 id="liability-modal-title">{liability ? '編輯一般負債' : '新增一般負債'}</h2><p>房貸、車貸與其他負債會計入淨資產；每月付款會自動計入月支出。</p></div><button type="button" className="icon-button" aria-label="關閉視窗" onClick={onClose}><X size={19} /></button></div>
        <form className="asset-form" onSubmit={(event) => void handleSubmit(event)}>
          <div className="form-grid form-grid-two">
            ...
            <FormField label="目前餘額" hint="TWD"><input required min="0" step="any" type="number" value={draft.outstandingBalance || ''} onChange={(event) => setDraft((current) => ({ ...current, outstandingBalance: Number(event.target.value) }))} placeholder="0" /></FormField>
  ...
  return (
    <div className="liability-panel">
      <section className="card risk-overview-card">
        <div className="risk-overview-top"><div><div className="section-kicker">家庭負債總覽</div><h2>房貸與一般負債，完整放進家庭資產負債表。</h2><p className="risk-overview-description">目前餘額會從總資產扣除；每月付款會自動加入現金流，不需要再重複手動登錄。</p></div><div className="risk-overview-ratio"><div className="risk-overview-ratio-label"><Landmark size={16} />每月負債支出</div><strong className="risk-overview-value">{formatTwd(monthlyPayment, displayMode)}</strong><span className="risk-badge status-warning">{activeLiabilities.length} 筆啟用中</span></div></div>
        <div className="risk-overview-stats"><div className="risk-overview-stat"><span>一般負債餘額</span><strong>{formatTwd(totalBalance, displayMode)}</strong></div><div className="risk-overview-stat"><span>每月付款</span><strong>{formatTwd(monthlyPayment, displayMode)}</strong></div><div className="risk-overview-stat"><span>房貸筆數</span><strong>{activeLiabilities.filter((item) => item.type === 'mortgage').length} 筆</strong></div><div className="risk-overview-stat"><span>未計入筆數</span><strong>{liabilities.length - activeLiabilities.length} 筆</strong></div></div>
      </section>
      ...
      <div className="formula-note"><span className="formula-note-mark">Σ</span><span><strong>計算原則：</strong>一般負債餘額會計入總負債；每月付款會計入月支出。若已建立房貸資料，現金流頁不需再重複新增同一筆房貸。</span></div>
