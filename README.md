...
V0.1 的基線不把借款當成已存在的資產；V0.2 已加入實際借款與維持率管理，V0.3 再加入不寫入資料庫的借款再投入試算，V0.4 加入市場壓力測試，V0.5 接上每月現金流管理，V0.6 把被動收入目標換算成所需本金與股數，V0.7 再把多個借款／投資方案放在同一張比較表，V1.0 補齊本機 JSON 備份與完整手機使用流程。這能確保「總資產」與「淨資產」不被借款誤算混淆。
