...
「投資資產 × 股票質押 × 現金流 × 被動收入」模擬 APP。第一階段是手機優先、離線優先的個人投資資產負債表，讓使用者先建立股票與現金基準線，再逐步加入質押、槓桿風控與現金流規劃。
...
V0.1 尚未把借款當成已存在的資產；質押模擬、維持率壓力測試、現金流項目與被動收入目標會依 V0.2–V0.6 逐步加入。這能確保「總資產」與「淨資產」不被借款誤算混淆。
